...
│   ├── compressed-ply-loader.ts SuperSplat compressed PLY (element chunk + packed_* fields)
...
#### Tier 0 — Already Present in `public/models/` (Must Support)

One additional PLY variant was found in the project's own test assets:

| Format | Ext | Bytes/Gaussian | Notes |
|--------|-----|---------------|-------|
| **SuperSplat Compressed PLY** | `.compressed.ply` | 16 B | Generated by [SuperSplat](https://github.com/playcanvas/supersplat); ~5× smaller than standard PLY; chunk-quantized uint32 fields |

**Detection**: PLY magic bytes + header contains **both** `element chunk` and `property uint packed_position`.

**Files confirmed in `public/models/`**:

| File | Chunks | Gaussians | File size |
|------|--------|-----------|-----------|
| `garden.compressed.ply` | 22,793 | 5,834,781 | 91 MB |
| `eglise_chenille_3840_4000_210.compressed.ply` | 15,523 | 3,973,688 | 62 MB |
| `central2_splat3_1_00.compressed_copyright.ply` | 3,872 | 991,089 | 15 MB |

Each chunk covers exactly **256 Gaussians**.

**Binary layout** after `end_header`:

```
[Chunk table]   n_chunks × 18 × float32   (72 bytes/chunk)
[Vertex data]   n_verts  ×  4 × uint32    (16 bytes/Gaussian)
```

**Chunk table** — 18 `float32` per chunk, bounding the 256 Gaussians it covers:

```
min_x/y/z,  max_x/y/z        ← world-space position bounds
min_sx/y/z, max_sx/y/z       ← log-space scale bounds
min_r/g/b,  max_r/g/b        ← linear color channel bounds (no per-chunk opacity bounds)
```

**Vertex packing** — 4 `uint32` per Gaussian (verified by decoding live files):

| Field | Bit layout | Encoding |
|-------|-----------|----------|
| `packed_position` | `X[31..21] Y[20..11] Z[10..0]` | 11+10+11 unorm; denorm via `lerp(chunk_min, chunk_max)` |
| `packed_rotation` | `mode[31..30] A[29..20] B[19..10] C[9..0]` | 2+10+10+10; smallest-three quaternion; `mode` = index of omitted largest component |
| `packed_scale` | `SX[31..21] SY[20..11] SZ[10..0]` | 11+10+11 unorm; denorm via `lerp(chunk_min, chunk_max)`; values are already log-space |
| `packed_color` | `R[31..24] G[23..16] B[15..8] A[7..0]` | 8+8+8+8; RGB denorm via `lerp(chunk_min, chunk_max)`; A is raw uint8 opacity |

**Decompression** — CPU-side, in `loader/compressed-ply-loader.ts`:

```typescript
function unorm(val: number, bits: number): number {
  return (val & ((1 << bits) - 1)) / ((1 << bits) - 1);
}

function decodeVertex(pp: number, pr: number, ps: number, pc: number,
                      chunk: Float32Array): GaussianFields {
  // Position
  const pos = [
    chunk[0] + unorm(pp >>> 21, 11) * (chunk[3] - chunk[0]),
    chunk[1] + unorm(pp >>> 11, 10) * (chunk[4] - chunk[1]),
    chunk[2] + unorm(pp,        11) * (chunk[5] - chunk[2]),
  ];

  // Scale (log-space — no extra log() needed)
  const scale = [
    chunk[6]  + unorm(ps >>> 21, 11) * (chunk[9]  - chunk[6]),
    chunk[7]  + unorm(ps >>> 11, 10) * (chunk[10] - chunk[7]),
    chunk[8]  + unorm(ps,        11) * (chunk[11] - chunk[8]),
  ];
  ...
  const rotation = [[m,a,b,c],[a,m,b,c],[a,b,m,c],[a,b,c,m]][mode]; // [w,x,y,z]

  // Color → SH DC coefficient
  const SH_C0 = 0.28209479177387814;
  const r  = chunk[12] + ((pc >>> 24) / 255) * (chunk[15] - chunk[12]);
  const g  = chunk[13] + (((pc>>>16)&0xFF) / 255) * (chunk[16] - chunk[13]);
  const b_ = chunk[14] + (((pc>>>8)&0xFF)  / 255) * (chunk[17] - chunk[14]);
  const shDC = [(r-0.5)/SH_C0, (g-0.5)/SH_C0, (b_-0.5)/SH_C0];

  // Opacity: raw uint8 → logit
  const alpha = (pc & 0xFF) / 255;
  const opacityLogit = Math.log(alpha / (1 - alpha + 1e-10));

  return { pos, logScale: scale, rotation, opacityLogit, shDC };
}
```

Output is the same `GaussianSceneData` as standard PLY — zero changes needed downstream.
...
| **SPLAT** | `.splat` | 32 B | 0 (DC only) | antimatter15 binary; no header; most common web format today |
...
async function loadScene(file: File): Promise<SceneData> {
  ...
  if (u8[0] === 0x70 && u8[1] === 0x6C && u8[2] === 0x79 && u8[3] === 0x0A) {
    // PLY family — inspect header further to pick the right sub-parser
    const headerText = new TextDecoder().decode(new Uint8Array(buf, 0, 2048));
    if (headerText.includes('element chunk') && headerText.includes('packed_position')) {
      return parseCompressedPLY(buf);  // SuperSplat compressed PLY
    }
    if (headerText.includes('f_dc_0')) {
      return parseGaussianPLY(buf);    // Standard 3DGS PLY (Gaussian splat)
    }
    return parsePointCloudPLY(buf);    // Plain point cloud PLY
  }
  if (u8[0] === 0x1F && u8[1] === 0x8B) {
    return parseSPZ(buf);              // gzip magic → SPZ
  }
  if (new TextDecoder().decode(u8) === 'KSpl') {
    return parseKSplat(buf);           // KSPLAT magic
  }
  // Fallback: assume .splat binary (no magic bytes, fixed 32-byte stride)
